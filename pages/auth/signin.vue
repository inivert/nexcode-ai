  ...
  <div class="relative min-h-screen w-full overflow-hidden bg-black">
    ...
    >
      <div class="relative w-full max-w-md px-4 py-8 sm:px-6 lg:px-8">
        <div 
          class="group relative rounded-3xl border border-white/10 bg-black/40 p-8 backdrop-blur-xl transition-all duration-500 hover:border-white/20 hover:bg-black/60 hover:shadow-[0_0_1000px_0_rgba(192,38,211,0.05)] hover:shadow-pink-500/5"
          ...
          <div class="relative space-y-6">
            <div class="text-center">
              ...
              <p class="mt-3 text-sm text-gray-400">
                ...
                <NuxtLink to="/auth/signup" class="font-medium text-pink-500 hover:text-pink-400 transition-colors duration-300">
                  Sign up
                </NuxtLink>
              </p>
            </div>
...
const cardStyle = computed(() => ({
  ...
  transition: rotation.value.x === 0 && rotation.value.y === 0 ? 'all 0.5s ease-out' : 'none'
}))

function handleMouseMove(e: MouseEvent) {
  if (!card.value) return
  
  const rect = (card.value as HTMLElement).getBoundingClientRect()
